...
def build_features_for_csv(in_csv: Path, out_dir: Optional[Path] = Path("../../data/feature_extraction")) -> Path:
    """
    Reads the full candidates CSV (no chunking), computes ONLY selected features, and writes to data/feature_extraction/.
    """
    in_csv = Path(in_csv)
    out_dir = Path(out_dir) if out_dir is not None else Path("../../data/feature_extraction")
...
if __name__ == "__main__":
    IN_CANDIDATES = Path("../../data/blocking/er_blocking_candidates_k40.csv")
